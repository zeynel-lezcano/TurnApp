...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  // Apply rate limiting for config endpoint
  const rateLimitResponse = await rateLimitMiddleware(request, 'api.config', context);
  if (rateLimitResponse) {
    return rateLimitResponse;
  }

  try {
    // Create cache key for shop config
    const cacheKey = CacheKeys.config(context.shop);

    // SCHRITT 2-5: Config mit Caching laden (5 min TTL)
    const validatedConfig = await withCache(
      cacheKey,
      async () => {
        ...
        return ConfigResponseSchema.parse(configResponse);
      },
      CacheTTL.CONFIG // 5 minutes TTL
    );

    // Handle shop not found error
    if (!validatedConfig) {
    ...
    if (error instanceof Error && error.message === "Shop not found or not active") {
      return json({ 
