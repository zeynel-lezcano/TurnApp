...
function decryptToken(ciphertext) {
  try {
    ...
    if (hmac.update(iv), hmac.update(encrypted), !hmac.digest().equals(providedHmac))
      throw new Error("Token integrity check failed - Daten wurden manipuliert oder Schl\xFCssel ist falsch");
...
async function getShopSettings(shopDomain) {
  try {
    let shop = await prisma.shop.findUnique({
      ...
      select: {
        settings: !0,
        // Nur Settings laden (kein Token für Sicherheit)
        uninstalledAt: !0
        // Status-Check für Installation
      }
    });
...
function verifySessionToken(token) {
  try {
    ...
    if (!verifyJWTSignature(token, apiSecret))
      return console.error("Session token signature verification failed - Token m\xF6glicherweise gef\xE4lscht"), null;
    let decoded = decodeJWT(token);
    if (!decoded)
      return console.error("JWT payload decoding failed - malformed token"), null;
    if (!decoded.iss || !decoded.dest || !decoded.aud)
      return console.error("Invalid session token: missing required fields (iss, dest, aud)"), null;
    if (decoded.iss !== decoded.dest)
      return console.error(`Session token: iss (${decoded.iss}) and dest (${decoded.dest}) don't match - possible token manipulation`), null;
    let currentTimestamp = Math.floor(Date.now() / 1e3);
    return decoded.exp && decoded.exp < currentTimestamp ? (console.error(`Session token expired: exp=${decoded.exp}, now=${currentTimestamp}`), null) : decoded;
  } catch (error) {
...
function createSessionMiddleware() {
  return async (request) => {
    ...
    return session ? {
      shop: session.iss,
      // Shop Domain als String für einfache Verwendung
      session
      // Vollständiger Token für erweiterte User-Info
    } : null;
  };
}
...
import { z } from "zod";
var ShopDomainSchema = z.string().regex(/^[a-z0-9-]+\.myshopify\.com$/, "Invalid shop domain format - must be valid Shopify domain"), HexColorSchema = z.string().regex(/^#[0-9A-F]{6}$/i, "Invalid hex color format - must be #RRGGBB"), UrlSchema = z.string().refine((url) => {
  if (url === "")
    return !0;
  try {
...
}, "Invalid or unsafe URL - only http/https allowed").optional().or(z.literal("")), BrandingSettingsSchema = z.object({
  // Shop-Name: Alphanumerisch + Leerzeichen, Bindestriche, Underscores
  brandName: z.string().min(1, "Brand name is required").max(50, "Brand name too long").regex(/^[a-zA-Z0-9\s\-_]+$/, "Brand name contains invalid characters - only letters, numbers, spaces, hyphens, underscores allowed"),
